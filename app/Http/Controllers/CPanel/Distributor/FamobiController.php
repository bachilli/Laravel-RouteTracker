...
class FamobiController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($feed->games as $game) {
            ...
            $publication = Publication::where('key', $key)
                ->where('distributor_id', self::FAMOBI_DISTRIBUTOR_ID)
                ->where('type', 'GAME')->first();

            if (empty($publication)) {
                Publication::create([
                    'key' => $key,
                    'distributor_id' => self::FAMOBI_DISTRIBUTOR_ID,
                    'name' => $game->name,
                    'type' => 'GAME',
                    'status' => 'PENDING',
                    'data' => [
